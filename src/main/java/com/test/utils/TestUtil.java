...
public class TestUtil extends TestBase {
	...
	public static boolean validateAttribute(WebElement we, String attrubuteName, String expectedAttrValue) {
		...
		TestBase.log.info("Validate an attribute name " + attrubuteName + " expected value " + expectedAttrValue
	...
	public static void sendKeyFromKeyboard(Keys key, WebElement we) {
		...
		TestBase.log.info("Sending Key from keyboard " + key);
		we.sendKeys(key);
	}
	...
	public static void clear(WebElement we) {
		TestBase.log.info("Send CTRL+A and DELETE on element");
	...
	/*
	 * Method to take screenshots at the end of test
	 * 
	 * @param we
	 * 
	 * @param elementName
	...
	public static void checkDataInResult(List<String> dataList, String searchKeyword) {

		List<String> dataListOne = dataList;

		for (int i = 0; i < dataListOne.size(); i++) {
			if (dataListOne.get(i).toLowerCase().contains(searchKeyword)) {
				log.info("Search keyword found .....");
			} else {
				System.out.println("Search keyword not found" + " :: " + dataList.get(i));
			}
		}
	}

	public static void selectByVisibleTextFromDropDown(WebElement wb, String data) {

		Select drpdown = new Select(wb);
		drpdown.selectByVisibleText(data);
	}
	
	public static boolean ifImageExist(String imagePath)
	{
		try {
			...
			{
				log.info("Expected Image found || Path : "+imagePath);
				return true;
			}
			else
					{
				System.out.println("Image not Found");
				log.info("Expected image not found || Path : "+imagePath);
				return false;
					}
		} catch (Exception e) {
	...
	public static ArrayList<Object[]> getdatafromexcel()
	{
		...
		String userDriectory=System.getProperty("user.dir");
		System.out.println(userDriectory);
		try {
			File file=new File(userDriectory+"\\src\\main\\resources\\feeddata.xlsx");
		    ...
		    for (int i = 0; i < rowCount+1; i++) {
		    	...
		    	int colCount= sheet.getRow(i).getLastCellNum()-sheet.getRow(i).getFirstCellNum();
		        for (int j = 0; j < colCount+1; j++) {
		        	ob[j]=sheet.getRow(i).getCell(j).toString();
		        }
				/*
				 * String firstname=sheet.getRow(i).getCell(0).toString(); String
				 * lastname=sheet.getRow(i).getCell(1).toString(); String
				 * rollno=sheet.getRow(i).getCell(2).toString(); String
				 * department=sheet.getRow(i).getCell(3).toString(); String ob[]=
				 * {firstname,lastname,rollno,department};
				 */
